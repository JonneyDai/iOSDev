...
// Data 2
    ...
    if ([mode isEqualToString:KAirCAutoMode]) {             //自动
        ...
        data |= 0x20 & 0x70;
    }else if([mode isEqualToString:KAirCDryMode]){          //送风
        data |= 0x30 & 0x70;
    }else if([mode isEqualToString:KAirCDryMode]){          //制热
        data |= 0x40 & 0x70;
    }else{
        data |= 0x00;
    }
...
//        [mAcDB UpDataDBDidEnd];
        //notification通知出去
        [[NSNotificationCenter defaultCenter] postNotificationName:@"StoreHaveChange" object:nil];
